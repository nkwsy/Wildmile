import { Title, Paper, Textarea, NumberInput, Select, Button, Group, Container, SimpleGrid, rem } from '@mantine/core'
  ...
  const plant_locations = props.module.plants.map((plant) => {
    ...
    return (
      <Paper key={`${plant.x}-${plant.y}`}>
        ...
        {(props.plants[plant.plant].common_name || props.plants[plant.plant].scientific_name || props.plants[plant.plant].scientificName)}
      </Paper>
    )
  }) 
            ...
            <form onSubmit={form.onSubmit((values) => { console.log(values) })}>
              ...
              <Select
                ...
                data={[
                  ...
                  { value: 'Dock-20 foot angled', label: 'Dock-20 foot angled' },
...
export async function getServerSideProps(context) {
  ...
  const module_plant_query = await IndividualPlant.find({ module: module_id }, ['-createdAt', '-updatedAt', '-module']).sort('x')
  ...
  const plants = plants_unmapped.reduce((obj, item) => (obj[item._id] = item, obj) ,{})
